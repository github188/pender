/*对webuploader进行封装，实现一些基本的功能*/
function uploadCreater(option) {

    var $wrap = $(option.container||'.uploader'),
    ...
    uploader = WebUploader.create({
        ...
        dnd: (option.container||'.uploader')+' .queueList',
        paste: document.body,
        ...
        fileNumLimit: option.fileNumLimit||100
    ...
    function setState( val ) {
        ...
        switch ( state ) {
            ...
            case 'finish':
                stats = uploader.getStats();
                if ( stats.successNum ) {
                    option.submitForm&&option.submitForm();
                } else {
                    // 没有成功的图片，重设
                    state = 'done';
                    location.reload();
...
var uploadFun = {
    addFile: function(file, wrap, uploader) {
        $item = $('<div class="file-item"></div>');
        $inner = $('<div class="file-inner">预览生成中……</div>');
        $delIcon = $('<span style="float:right;">X<span>');
