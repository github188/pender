...
<script type="text/javascript">
	...
	$(function() {
		...
		$.ajax({
			url: '${pageContext.request.contextPath}/store/overview/findCyclicOrgnizations.json',
			task: function(r) {
				var orgList = r.orgnizationList;
				$('#q_point_org').combobox({
					data: orgList
				})
				$('#pointOrgId').combobox({
					data: orgList
		...
		$('#pointGrid').datagrid({
			...
			singleSelect: true,
			idField:'id',
			url:'${pageContext.request.contextPath}/store/overview/find.json',
	...
	function initNotBindDeviceGrid() {
		$('#notBindDeviceGrid').datagrid({
            ...
            singleSelect: true,
            pagination:false,
            url: '${pageContext.request.contextPath}/store/overview/findDevCombination.json',
	...
	function savePoint() {
		...
		$.ajax({
			url: '${pageContext.request.contextPath}/store/overview/save.json',
			dataType:"json",
	...
	function delPoints(id) {
		confirmMsg('删除店铺，请先解绑设备<br>是否确认删除该店铺？', function() {
			$.ajax({
				url : '${pageContext.request.contextPath}/store/overview/delete.json',
				data : $.param({
					'ids': [id]
				}, true),
				info: '所选店铺删除成功！',
	...
	function saveDevice() {
		...
		$.ajax({
			...
			url: '${pageContext.request.contextPath}/store/overview/addDevCombination.json',
			success: function() {
	...
	function doUnbindDevices(identity) {
		$.ajax({
			...
			url: '${pageContext.request.contextPath}/store/overview/deleteDevCombination.json',
			success: function() {
				$('#deviceGrid').datagrid('reload');
			}
		})
    ...
    function showPointOperate(value, row) {
		...
		+'<a href="javascript:void(0)" class="icon-remove easyui-tooltip" data-options="content:\'删除\'" onclick="delPoints('+row.id+')" style="display:inline-block;width:16px;height:16px;"></a>';
		return operate;
    }
...
<body class="easyui-layout" >
	<div data-options="region:'north',border:false,split:true" style="padding:15px; height:84px;">
		...
		<form id="pointQueryForm" class="search-form">
			...
			<div class="form-item">
				<div class="text">所属组织</div>
				<div class="input">
					<input id="q_point_org" name="orgId" class="easyui-combobox" data-options="prompt:'所属组织',valueField:'id',textField:'name'" />
				</div>
			</div>
			<div class="form-item">
	...
	<div data-options="region:'center',border:false,headerCls:'list-head',tools:'#pointOpt'" title="店铺信息" style="width:60%;">
		<div id="pointOpt">
			<sec:authorize access="add,save"><a href="javascript:void(0)" class="icon-add easyui-tooltip" data-options="content:'新增店铺'" onclick="addPoint()"></a></sec:authorize>
		</div>
			...
			<table id="pointGrid">
				<thead>
					<tr>
						...
						<th data-options="field:'orgName',width:150" align="center">所属组织</th>
	...
	<div id="winPoint" class="easyui-dialog" data-options="closed:true, buttons:'#pointBtns', onClose:resetPointForm" style="width:429px;padding:10px 20px;">
		<form id="fomPoint" method="post">
			...
			<div class="form-row">
				...
				<div class="form-cloumn form-item">
					<div class="text">所属组织</div>
					<input id="pointOrgId" name="orgId" class="easyui-textbox" class="easyui-combobox" data-options="prompt:'所属组织',required:true,valueField:'id',textField:'name'">
				</div>
			</div>
			<div class="form-row">
